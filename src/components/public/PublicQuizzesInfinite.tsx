...
import PublicQuizCard, { PublicQuiz } from "@/components/public/PublicQuizCard";

export type PublicFilter = "active" | "expired";

export default function PublicQuizzesInfinite({
  filter,
  pageSize = 15,
}: {
  filter: PublicFilter;
  ...
  const [items, setItems] = useState<PublicQuiz[]>([]);
    ...
    <>
      ...
      {items.length > 0 && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {items.map((q) => (
            <PublicQuizCard key={q.id} quiz={q} />
          ))}
        </div>
      )}
